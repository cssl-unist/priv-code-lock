...
u64 __m_s module_emit_got_entry(struct module *mod, u64 val)
{
...
u64 __m_s module_emit_plt_entry(struct module *mod, u64 val)
{
...
static int __m_s is_rela_equal(const Elf64_Rela *x, const Elf64_Rela *y)
{
	return x->r_info == y->r_info && x->r_addend == y->r_addend;
}

static bool __m_s duplicate_rela(const Elf64_Rela *rela, int idx)
{
	int i;
	for (i = 0; i < idx; i++) {
...
static void __m_s count_max_entries(Elf64_Rela *relas, int num,
...
int __m_s module_frob_arch_sections(Elf_Ehdr *ehdr, Elf_Shdr *sechdrs,
