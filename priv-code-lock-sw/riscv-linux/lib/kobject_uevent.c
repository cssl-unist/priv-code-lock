...
static int __m_s init_uevent_argv(struct kobj_uevent_env *env, const char *subsystem)
{
	int len;
...
static int __m_s kobject_uevent_net_broadcast(struct kobject *kobj,
					struct kobj_uevent_env *env,
...
static void __m_s zap_modalias_env(struct kobj_uevent_env *env)
{
 ...
 * corresponding error when it fails.
 */
int __m_s kobject_uevent_env(struct kobject *kobj, enum kobject_action action,
		       char *envp_ext[])
{
	struct kobj_uevent_env *env;
	...
	/* environment buffer */
	env = kzalloc(sizeof(struct kobj_uevent_env), GFP_KERNEL); 	//TODO:pcl-alloc
	if (!env)
		return -ENOMEM;
	...
	mutex_lock(&uevent_sock_mutex); 	//TODO:pcl-schedule
 ...
 * corresponding error when it fails.
 */
int __m_s kobject_uevent(struct kobject *kobj, enum kobject_action action)
{
 ...
 */
int __m_s add_uevent_var(struct kobj_uevent_env *env, const char *format, ...)
{
	va_list args;
	int len;
