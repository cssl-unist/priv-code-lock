...
class vpoffsetCfg(implicit p: Parameters) extends CoreBundle()(p) {
  require(paddrBits > 12)
  val offset = UInt(width = xLen)
}
...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  // for vpoffset
  val vpoffset_cfg = RegInit(0.U.asTypeOf(new vpoffsetUpdate))
  val requestedVPN = Reg(UInt(width = vpnBits))
  if(hasPrivCodeLock) { 
  
  when(io.ptw.vpoffset.valid) {
        vpoffset_cfg.value := io.ptw.vpoffset.bits.value
  }
...
when (do_refill && !invalidate_refill) {
    ...
    // vp offset check
    val r_vp_offset = pte.ppn + vpoffset_cfg.value
    //val r_vp_offset =  requestedVPN - pte.ppn // vpn-ppn
    //val r_pv_offset = pte.ppn - requestedVPN // vpn-ppn
    
    def vp_matched = Mux(pte.u || (vpoffset_cfg.value === 0.U) || io.ptw.resp.bits.level =/=  pgLevels-1, pte.sx(), Mux(r_vp_offset === requestedVPN, pte.sx(), Bool(false))) 
  //  def pv_matched = Mux(pte.u || (vpoffset_cfg.value === 0.U) || io.ptw.resp.bits.level =/= pgLevels-1, pte.sx(), Mux(r_pv_offset ===  vpoffset_cfg.value, pte.sx(), Bool(false)))
    ...
    newEntry.sr := pte.sr()
    newEntry.sw := pte.sw()   
    newEntry.sx := (vp_matched)// || pv_matched) // vp check
    newEntry.pr := prot_r
    newEntry.pw := prot_w
    newEntry.px := prot_x 
  ...
  if (hasPrivCodeLock) {
    when(io.ptw.req.bits.valid) {
      requestedVPN := io.ptw.req.bits.bits.addr
    }
  }
