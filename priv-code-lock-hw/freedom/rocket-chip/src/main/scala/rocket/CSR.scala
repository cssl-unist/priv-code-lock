...
class PCodeCSR(implicit p: Parameters) extends CoreBundle()(p) {
...
class PCodeUpdate(implicit p: Parameters) extends CoreBundle
with HasCoreParameters {
  val id = UInt(OUTPUT, log2Ceil(numPCodeRanges))
  val value = new PCodeCSR().asOutput
}

class vpoffsetUpdate(implicit p: Parameters) extends CoreBundle
with HasCoreParameters {
  val value = UInt(width=vpnBits).asOutput
}
...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
   ...
   * priv-code-block
   */
  val pcode_req = Valid(new PCodeUpdate)
  val pcode_resp = Valid(new PCodeUpdate).flip
  //val pcode_debug = DecoupledIO(Bits(width=paddrBits)).flip
  
  val vpoffset_req = Valid(new vpoffsetUpdate)
  val vpoffset_resp = Valid(new vpoffsetUpdate).flip
...
class CSRFile(
   ...
   * priv-code-block
   *
   * csrw -> pcode_update -> (PTW) -> pcode_update_resp -> pcode_ranges
   */
  val vpoffset_reg = Reg(UInt(width = vpnBits))
  

  val pcode_regs = Reg(init=Vec.fill(numPCodeRanges){new PCodeCSR().fromBits(0)})
  def pcode_csrs = Seq(
    CSRs.spcoderange0
  ,CSRs.spcoderange1
  ,CSRs.spcoderange2
 ...
 /* 
  val isolation_reg = Reg(new isolationCSR())
  // For LKM read interface
  if(hasPrivCodeLock) {
    read_mapping += CSRs.isolationsw -> (isolation_reg.isolationsw)
  }
*/
  val pcode_update = Reg(Valid(new PCodeUpdate))
  val pcode_update_resp = Wire(Valid(new PCodeUpdate))
  
  val vpoffset_update = Reg(Valid(new vpoffsetUpdate))
  val vpoffset_update_resp = Wire(Valid(new vpoffsetUpdate))
  println(s"hasPrivCodeLock: ${hasPrivCodeLock}")
  if (hasPrivCodeLock) {
    
    
    read_mapping += CSRs.vpoffset -> vpoffset_reg
    vpoffset_update_resp := io.vpoffset_resp
    io.vpoffset_req := vpoffset_update
    require(numPCodeRanges <= 4 && numPCodeRanges <= pcode_csrs.length)
    
    pcode_update_resp := io.pcode_resp
    io.pcode_req := pcode_update 
    

    when(pcode_update_resp.valid) {
      pcode_regs(pcode_update_resp.bits.id) := pcode_update_resp.bits.value
    }


    for (csr_id <- pcode_csrs) {
      read_mapping += csr_id -> ( 
      {
        //val r = pcode_regs(csr_id & 3)
        val r = pcode_regs(csr_id & numPCodeRanges-1)
        Cat(r.base, r.mask, r.valid, r.locked)
      }
  ...
  if(hasPrivCodeLock) {
    pcode_update.valid := Bool(false)
    vpoffset_update.valid := Bool(false)
    //io.pcode_debug.ready := csr_wen && decoded_addr(CSRs.slockpcode)
    //printf("csr_wen: %x; decoded: %x; addr: %x; ref: %x, ref2: %x\n",
    //  csr_wen, decoded_addr(CSRs.spcoderange0), io.rw.addr,
    //  decoded_addr(0x190),
    //  decoded_addr(CSRs.spcoderange0)
    //  )
  }

  when (csr_wen) {
    if(hasPrivCodeLock) {
      when(decoded_addr(CSRs.vpoffset)) {
        vpoffset_update.valid := Bool(true)
        vpoffset_reg := wdata(vaddrBits-1, pgIdxBits)
        vpoffset_update.bits.value := wdata(vaddrBits-1, pgIdxBits) 
        
       }
      for (pcode_csr <- pcode_csrs) {
        // pcode_csrs = 0x190, 0x191, 0x192, 0x193. take the two LSBs
        when (decoded_addr(pcode_csr)) {
          val id = pcode_csr & UInt(3, width=2)
          val lock = pcode_regs(id).locked
          //printf("pcode regs %x(%x): %x\n",id, pcode_csr, pcode_regs(id).locked)
          when(lock === UInt(0)) {
            pcode_update.valid := Bool(true)
            pcode_update.bits.id := id
            pcode_update.bits.value.base := wdata(paddrBits-1, 12)
            pcode_update.bits.value.mask := wdata(11,2)
            pcode_update.bits.value.valid := wdata(1)
            pcode_update.bits.value.locked := wdata(0)
            //printf("setting pcode, id: %x (%x), val: %x\n",id,
            //  pcode_csr,
            //  wdata)
            pcode_regs(id).locked := wdata(0) // eagerly update the lock
          }
        }
      }
      //For LKM - write interface: isolationsw 
      //isolation_reg.isolationsw := wdata(0)
