...
case class RocketCoreParams(
  ...
  override val hasPrivCodeLock: Boolean = false,
  override val numPCodeRanges: Int = 8
) extends CoreParams {
...
class RocketCustomCSRs(implicit p: Parameters) extends CustomCSRs with HasRocketCoreParameters {
  ...
  // Past releases: <none>
  // Set bit[30] to 1 (0010 -> 0100) to indicate that this implements PCL
  /*
   * priv-code-block.
   * not tested.
   */
  def datestamp = 0x20181004
  def mimpidVal = if (hasPrivCodeLock) {
    0x04000000 & datestamp
  } else {
    datestamp
  }
  def mimpid = CustomCSR.constant(CSRs.mimpid, BigInt(mimpidVal))
...
class Rocket(implicit p: Parameters) extends CoreModule()(p)
   ...
   * priv-code-lock
   */
  io.ptw.pcode_req := csr.io.pcode_req
  csr.io.pcode_resp := io.ptw.pcode_resp
  //csr.io.vpoffset_resp := io.ptw.vpoffset_resp
  io.ptw.vpoffset_req := csr.io.vpoffset_req
  //csr.io.pcode_debug <> io.ptw.pcode_debug
  //io.tlb.vpoffset_req := csr.io.vpoffset_req
