...
INLINE static void set_pcl_pa(uintptr_t base, uintptr_t mask, uint8_t idx) {
    ...
    switch(idx) {
        case 0: write_csr(0x191,mask & 0xFFFF); break;
        case 1: write_csr(0x193,mask & 0xFFFF); break;
        case 2: write_csr(0x195,mask & 0xFFFF); break;
        case 3: write_csr(0x197,mask & 0xFFFF); break;
        default: __builtin_unreachable();
    }
...
int main( int argc, char* argv[] )
{
    ...
    //set_pcl(SCRATCH, 0xFFFF);
    set_pcl_pa((uintptr_t)scratch, 0xFFFF, 0);
    printf("(3) set testcode to be pcode\n");
    set_pcl_pa(0x80000000, 0xFFFE, 1);
    printf("(4) enable pcl\n");
    enable_pcl();
    printf("(5) touch scratch\n");
    test_one_s((uintptr_t)scratch+8,4);
    printf("test done.\n");
    ...
    printf("%x\n",read_csr(mstatus));
    printf("sstatus: %x\n",read_csr(sstatus));
    uintptr_t new_mstatus = (read_csr(mstatus) & ~MSTATUS_MPP) | (MSTATUS_MPP & 0x1);
